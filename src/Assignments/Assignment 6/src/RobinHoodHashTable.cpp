...
bool RobinHoodHashTable::insert(const string& elem) {
    if(the_size==numSlots) return false;
    auto index=func(elem);
    Slot insert_elem;
    insert_elem.value=elem;
    insert_elem.isEmpty=false;
    while (!bucket[index].isEmpty) {
        if(bucket[index].value==elem) return false;
        if(insert_elem.distance>bucket[index].distance){
            swap(insert_elem,bucket[index]);
        }
        index++;
        insert_elem.distance++;
        if(index==numSlots) index=0;
        if(index==func(elem)) return false;
    }
    bucket[index]=insert_elem;
    the_size++;
    return true;
}
...
bool RobinHoodHashTable::contains(const string& elem) const {
    ...
    while (!bucket[index].isEmpty) {
        if(elem==bucket[index].value) return true;
        if(distance>bucket[index].distance)
            break;
        distance++;
        index++;
        if(index==numSlots) index=0;
...
bool RobinHoodHashTable::remove(const string& elem) {
    ...
    while (!bucket[index].isEmpty) {
        if(elem==bucket[index].value) {
            bucket[index].isEmpty=true;
            while (!bucket[(index+1)%numSlots].isEmpty &&
                   func(bucket[(index+1)%numSlots].value)!=(index+1)%numSlots) {
                bucket[(index)%numSlots]=bucket[(index+1)%numSlots];
                index++;
                bucket[index].isEmpty=true;
            }
            the_size--;
