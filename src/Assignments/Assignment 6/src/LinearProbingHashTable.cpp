...
LinearProbingHashTable::LinearProbingHashTable(HashFunction<std::string> hashFn) {
   ...
   bucket=new Slot[hashFn.numSlots()];
   numSlot=hashFn.numSlots();
   for(int i=0;i<hashFn.numSlots();++i) bucket[i].isEmpty=true;
   func=hashFn;
}
...
bool LinearProbingHashTable::insert(const string& elem) {
    /* TODO: Delete this comment and the next lines, then implement this function. */
    auto index=func(elem);

    if(the_size==numSlot) return false;
    if(contains(elem)) return false;
    while (!bucket[index].isEmpty ||bucket[index].hasTombstones){
        if(bucket[index].hasTombstones) {
            bucket[index].isEmpty=false;
            bucket[index].value=elem;
            the_size++;
            return true;
        }
        if(bucket[index++].value==elem ) return false;
        if( index==func(elem))  return false;
        if(index==numSlot) index=0;
    }
    bucket[index].isEmpty=false;
    bucket[index].value=elem;
    the_size++;
...
bool LinearProbingHashTable::contains(const string& elem) const {
    /* TODO: Delete this comment and the next lines, then implement this function. */
    auto index=func(elem);
    if(the_size==0) return false;
    while (!bucket[index].isEmpty || bucket[index].hasTombstones) {
        if(bucket[index].hasTombstones) {
            ++index;
            if(index==numSlot) index=0;
            if(index==func(elem)) break;
            continue;
        }
        if(bucket[index].value==elem) return true;
        ++index;
    ...
    return false;
//    if(the_size==0) return false;
//    for(auto index=func(elem);
//        !bucket[index].isEmpty ||bucket[index].hasTombstones;index++){
//        if(index==numSlot) index=0;
//        if(bucket[index].hasTombstones) continue;
//        if(bucket[index].value==elem) return true;
//        if(index+1==func(elem)) break;
//    }
//    return false;
}
...
bool LinearProbingHashTable::remove(const string& elem) {
    ...
    while (!bucket[index].isEmpty ||bucket[index].hasTombstones ) {
        ...
        if(bucket[index].value==elem ) {
            bucket[index].isEmpty=true;
            bucket[index].hasTombstones=true;
            --the_size;
            return true;
        }
        ++index;
...
void LinearProbingHashTable::printDebugInfo() const {
    ...
    for(int i=0;i<numSlot;++i){
        cout<<(bucket[i].isEmpty?"empty":"value =")<<" ";
        if(!bucket[i].isEmpty) cout<<bucket[i].value;
        cout<<"\n";
    }
}
