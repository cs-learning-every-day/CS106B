...
bool canAllPatientsBeSeen(const HashMap<string, int>& doctors,
    ...
    auto random_patient=patients.front();

    for(auto k:doctors.keys()){
        if(doctors[k]>=patients[random_patient]){
            auto doctors_c=doctors;
            auto patients_c=patients;
            doctors_c[k]-=patients[random_patient];
            patients_c.remove(random_patient);
            auto schedule_c=schedule;

//            schedule_c.put(k,{random_patient});
            schedule_c[k].add(random_patient);
            auto result=canAllPatientsBeSeen(doctors_c,patients_c,schedule_c);
            if(result==true) {
                schedule=schedule_c;
                return true;
            }
