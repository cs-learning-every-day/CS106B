...
Vector<DataPoint> combine(const Vector<Vector<DataPoint>>& sequences) {
    ...
    if(sequences.size()==1) return sequences.front();
    auto mid=sequences.size()/2;
    auto s1=combine(sequences.subList(0,mid));
    auto s2=combine(sequences.subList(mid));
    Vector<DataPoint> result;

    int i=0,j=0;
    while (i<s1.size()&&j<s2.size()) {
        result.add(s1[i].weight<s2[j].weight?s1[i++]:s2[j++]);
    }
    if(i!=s1.size()) result+=s1.subList(i);
    if(j!=s2.size()) result+=s2.subList(j);
    return result;
}
...
ADD_TEST("NULL casr test"){
    DataPoint a={"",0};
    auto merged=combine({{a}});
    EXPECT_EQUAL(merged[0],a);
}
