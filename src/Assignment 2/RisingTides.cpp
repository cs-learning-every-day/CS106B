...
Grid<bool> floodedRegionsIn(const Grid<double>& terrain,
    ...
    Grid<bool> res(nRow, nCol);
    Queue<GridLocation> q;

    // 初始化水源
    for (auto g : sources) {
        if (terrain.inBounds(g.row, g.col) &&
            !res[g.row][g.col] &&
            terrain[g.row][g.col] <= height) {
            res[g.row][g.col] = true;
            q.enqueue(g);
        }
    }
    ...
    while (!q.isEmpty()) {
        ...
        for (auto dir : directions) {
            int newRow = g.row + dir[0];
            int newCol = g.col + dir[1];
            if (terrain.inBounds(newRow, newCol) &&
                !res[newRow][newCol] &&
                terrain[newRow][newCol] <= height) {
                res[newRow][newCol] = true;
...
STUDENT_TEST("My Test") {
    ...
    sources.add({2, 2});
    water = floodedRegionsIn(world, sources, 3.0);
    expected = {
        { true, true, true },
        { true, false, true },
        { true, true, true }
