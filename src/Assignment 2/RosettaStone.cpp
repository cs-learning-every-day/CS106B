...
Map<string, double> kGramsIn(const string& str, int kGramLength) {
    if (kGramLength <= 0) {
        error("kGramLength must be postive!");
    }
...
Map<string, double> normalize(const Map<string, double>& input) {
    ...
    for (string k : input) {
        ...
        number += pow(input[k], 2);
    }
    if (!isOk) {
        error("input必须包含至少一个非零数");
    }

    for (string k : input) {
        res[k] = input[k] / sqrt(number);
    }
...
Map<string, double> topKGramsIn(const Map<string, double>& source, int numToKeep) {
    ...
    Map<string, double> res;
    PriorityQueue<string> pq; // 最小堆

    for (string k : source) {
        if (pq.size() == numToKeep) {
            if (source[k] > pq.peekPriority()) {
                pq.dequeue();
...
string guessLanguageOf(const Map<string, double>& textProfile,
    ...
    for (auto c : corpora) {
        double tmp = cosineSimilarityOf(textProfile, c.profile);
        if (tmp > consine) {
            res = c.name;
            consine = tmp;
        }
